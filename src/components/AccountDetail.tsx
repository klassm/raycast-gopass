...
export function AccountDetail({ accountKey }: { accountKey: string }) {
  ...
  return (
    <List isLoading={ loading } enableFiltering={ true }
          searchBarPlaceholder={ accountKey } throttle>
      { ( entries ).map((entry) => {
        if (data === undefined) {
          return <></>;
        }
        return (
          <GopassKeyValue key={ entry[0] } entryKey={ entry[0] } entryValue={ entry[1] } details={ data }/>
        );
      }) }
    </List>
  );
...
function GopassKeyValue({ entryKey, entryValue, details }: { entryKey: string, entryValue: string, details: AccountDetails }) {
  return (
    <List.Item
      title={ entryKey }
